...
SpeechEngine.prototype = {
  ...
  synthesize: function SpeechEngine_synthesize(aUtterance, aOptions, aCallback) {
    ...
    this.worker.postMessage({type: 'synthesize', utterance: aUtterance, options: aOptions || {}});
  },
...
AudioPlayer.prototype = {
  ...
  close: function AudioPlayer_close(aHandle) {
    ...
    let samplesToPlay = this.totalSamples - this.audioElement.mozCurrentSampleOffset();
...
var tts = {
  ...
  init: function tts_init(aAudioElement, aCallback) {
    ...
    this.earconEngine = new EarconEngine(aAudioElement.ownerDocument.defaultView);

    let engines = this.engines;
    let enginesToInitialize = this.builtinEngines.length;

    for each (let builtinEngine in this.builtinEngines) {
      let engine = new SpeechEngine(builtinEngine.worker, builtinEngine.dataPath);
      engine.init(
        function (success) {
          if (success) {
  ...
  _advanceQueue: function _advanceQueue() {
    ...
    switch (playTask.synthesizing) {
      ...
      case 'yes':
        ...
        break;
      default:
        playTask.engine.synthesize(playTask.utterance, playTask.options, playFunc);
        break;
    }
  },
