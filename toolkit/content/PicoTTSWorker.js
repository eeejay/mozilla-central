...
var PicoWrapper = {
  ...
  loadEngine: function loadEngine(aLanguage, aTaFile, aSgFile) {
    ...
    rv = this.addResourceToVoiceDefinition(this.System, this.PICO_VOICE_NAME, resourceName.readString());
    ...
    rv = this.newEngine(this.System, this.PICO_VOICE_NAME, this.Engine.address());
...
var PicoTTSWorker = {
  init: function init(aLanguagePath) {
    try {
      ...
      dump('Error initializing pico: ' + x + '\n');
      this._voices = [];
    }
  ...
  synthesize: function synthesize(aUtterance, aOptions) {
    ...
    if (aOptions.pitch != undefined) {
      let svoxPitch = Math.round(
        (aOptions.pitch >= 1) ?
          Math.min(aOptions.pitch * 100, 200) :
          Math.max(aOptions.pitch * 50 + 50, 50));
      utterance = '<pitch level="' + svoxPitch + '">' + utterance + '</pitch>';
    }

    if (aOptions.rate != undefined) {
      let svoxRate = Math.round(
        Math.min(Math.max(aOptions.rate * 100, 20), 500));
      utterance = '<speed level="' + svoxRate + '">' + utterance + '</speed>';
    }

    if (aOptions.volume != undefined) {
      let svoxVolume = Math.round(Math.min(100, aOptions.volume * 100));
      utterance = '<volume level="' + svoxVolume + '">' + utterance + '</volume>';
    }
    ...
    function feedEngine () {
      ...
      let rv = PicoWrapper.putTextUtf8(PicoWrapper.Engine, str, str.length, sentBytes.address());
    ...
    function getAudio (aMilliseconds) {
      ...
      while (true) {
        ...
        if (!sampleRate) {
          ...
          self.postMessage({type: 'audioInfo', sampleRate: sampleRate, channels: 1});
        }
  ...
  resetEngine: function resetEngine() {
    ...
    let rv = PicoWrapper.resetEngine(PicoWrapper.Engine, PicoWrapper.PICO_RESET_SOFT);
...
self.onmessage = function (event) {
  try {
    ...
    dump('Worker error: ' + x + ' (' + x.fileName + ':' + x.lineNumber + ':' + x.columnNumber +  ')\n');
    for (var attr in x)
      dump(' ' + attr + ': ' + x[attr] + '\n');
  }
};